  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-4 bg-gray-50">
      <div className="w-full max-w-2xl bg-white rounded-lg shadow-md p-8">
        <h1 className="text-3xl font-extrabold mb-4 text-center text-blue-900 tracking-tight">AI Syllabus Summarizer</h1>
        <p className="mb-6 text-gray-700 text-center text-lg">Paste the syllabus in Portuguese. The app will search the web and return a professional English summary for study.</p>
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          <textarea
            name="syllabus"
            value={syllabus}
            onChange={e => setSyllabus(e.target.value)}
            placeholder="Cole aqui o syllabus do curso (em português)..."
          ...
          <div className="flex gap-2">
            <button
              type="submit"
              className="bg-blue-600 text-white rounded px-4 py-2 font-semibold hover:bg-blue-700 transition flex-1"
...
function SummaryDisplay({ summary }: { summary: string }) {
  ...
  try {
    ...
    return (
      <div className="prose max-w-none mx-auto bg-gradient-to-br from-blue-50 to-white p-8 rounded-xl border border-blue-100 shadow-lg mt-8">
        <pre>{summary}</pre>
      </div>
    );
  }
      ...
      {parsed[activeTab] && (
        <div>
          <div className="text-gray-900 text-lg leading-relaxed font-sans mb-6 whitespace-pre-line">
            <div className="font-bold text-blue-900 mb-2">{parsed[activeTab].translated}</div>
            {parsed[activeTab].summary ? parsed[activeTab].summary.split(/\n\nSources:/)[0] : null}
          </div>
          {(() => {
